...
See here for examples of things we’ve come across while porting WCF projects to .NET Core. [link to page containing examples such as explicitly setting APIs to default values and overriding abstract methods for server side APIs that don’t exist in Core.]()
## Unsupported APIs
#### Known status
