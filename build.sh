...
prepare_managed_build()
{
    ...
    if [ ! -e "$__msbuildpath" ]; then
        ...
        mono --version
        echo mono "$__nugetpath" install $__msbuildpackageid -Version $__msbuildpackageversion -source "http://dotnet.myget.org/F/dotnet-buildtools/" -OutputDirectory "$__packageroot"
        mono "$__nugetpath" install $__msbuildpackageid -Version $__msbuildpackageversion -source "http://dotnet.myget.org/F/dotnet-buildtools/" -OutputDirectory "$__packageroot"
        if [ $? -ne 0 ]; then
