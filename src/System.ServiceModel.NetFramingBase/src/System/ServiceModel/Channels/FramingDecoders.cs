...
namespace System.ServiceModel.Channels
{
    ...
    internal abstract class StringDecoder
    {
        ...
        private Memory<byte> _encodedBytes;
            ...
            switch (_currentState)
            {
                case State.ReadingSize:
                    ...
                    if (_sizeDecoder.IsValueDecoded)
                    {
                        ...
                        if (_encodedBytes.IsEmpty || _encodedBytes.Length < _encodedSize)
                        {
                ...
                case State.ReadingBytes:
                    ...
                    else
                    {
                        ...
                        Memory<byte> slicedBytes = _encodedBytes.Slice(_encodedSize - _bytesNeeded, bytesConsumed);
                        ReadOnlySequence<byte> tempBuffer = buffer.Slice(0, bytesConsumed);
                        tempBuffer.CopyTo(slicedBytes.Span);
                        _bytesNeeded -= bytesConsumed;
                        if (_bytesNeeded == 0)
                        {
                            _value = Encoding.UTF8.GetString(_encodedBytes.Span);
        ...
        private static bool CompareBuffers(Memory<byte> buffer1, ReadOnlySequence<byte> buffer2)
        {
            byte[] buff = buffer2.ToArray();
            var buff1Span = buffer1.Span;
            for (int i = 0; i < buffer1.Length; i++)
            {
                if (buff1Span[i] != buff[i])
                {
                    return false;
                }
            }
            ...
            try
            {
                ...
                switch (CurrentState)
                {
                    ...
                    case ClientFramingDecoderState.ReadingFaultString:
                        bytesConsumed = _faultDecoder.Decode(buffer);
