...
namespace System.ServiceModel.Channels
{
    ...
    internal class ClientFramingDuplexSessionChannel : FramingDuplexSessionChannel
    {
        ...
        private async Task<IConnection> SendPreambleAsync(IConnection connection, Memory<byte> preamble, TimeSpan timeout)
        {
            ...
            Memory<byte> ackBuffer = new byte[1];
            ...
            try
            {
                ...
                if (!ConnectionUpgradeHelper.ValidatePreambleResponse(new ReadOnlySequence<byte>(ackBuffer.Slice(0, ackBytesRead)), _decoder, Via))
                {
    ...
    internal class ConnectionUpgradeHelper
    {
            ...
            Uri via, string contentType, TimeSpan timeout)
        {
            ...
            Memory<byte> faultBuffer = Fx.AllocateByteArray(FaultStringDecoder.FaultSizeQuota);
            int size = await connection.ReadAsync(faultBuffer, timeoutHelper.RemainingTime());
            ReadOnlySequence<byte> decodeBuffer = new ReadOnlySequence<byte>(faultBuffer.Slice(0, size));
            while (decodeBuffer.Length > 0)
            {
                int bytesDecoded = decoder.Decode(decodeBuffer);
                decodeBuffer = decodeBuffer.Slice(bytesDecoded);
                ...
                else
                {
                    ...
                    {
                        size = await connection.ReadAsync(faultBuffer, timeoutHelper.RemainingTime());
                        decodeBuffer = new ReadOnlySequence<byte>(faultBuffer.Slice(0, size));
                    }
                }
            }
            ...
            ClientFramingDecoder decoder, IDefaultCommunicationTimeouts defaultTimeouts, TimeSpan timeout)
        {
            ...
            while (upgradeContentType != null)
            {
                ...
                if (!ValidateUpgradeResponse(new ReadOnlySequence<byte>(buffer.Slice(0, size)), decoder)) // we have a problem
                {
        ...
        public static bool ValidatePreambleResponse(ReadOnlySequence<byte> buffer, ClientFramingDecoder decoder, Uri via)
        {
            if (buffer.Length == 0)
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
        ...
        private static bool ValidateUpgradeResponse(ReadOnlySequence<byte> buffer, ClientFramingDecoder decoder)
        {
            if (buffer.IsEmpty)
            {
