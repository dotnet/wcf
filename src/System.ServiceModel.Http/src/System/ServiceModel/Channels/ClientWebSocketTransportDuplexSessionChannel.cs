    ...
    internal class ClientWebSocketTransportDuplexSessionChannel : WebSocketTransportDuplexSessionChannel
    {
        ...
        protected internal override async Task OnOpenAsync(TimeSpan timeout)
        {
            ...
            try
            {
                ...
                {
                    var certificateProvider = await httpsChannelFactory.CreateAndOpenCertificateTokenProviderAsync(RemoteAddress, Via, channelParameterCollection, helper.RemainingTime());
                    var clientCertificateToken = await httpsChannelFactory.GetCertificateSecurityTokenAsync(certificateProvider, RemoteAddress, Via, channelParameterCollection, helper);
        ...
        private async Task<WebSocket> CreateWebSocketWithFactoryAsync(X509Certificate2 certificate, TimeoutHelper timeoutHelper)
        {
            ...
            try
            {
                ...
                {
                    headers[HttpRequestHeader.Host] = HttpTransportSecurityHelpers.GetIdentityHostHeader(RemoteAddress);
                }
