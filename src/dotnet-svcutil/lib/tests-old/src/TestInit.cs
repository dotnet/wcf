...
namespace SvcutilTest
{
    public partial class E2ETest
    {
        ...
        // TODO: move these fields into corresponding objects.

        // global variables common to all tests.
        private static string g_TestCasesDir;
        private static string g_TestResultsDir;
        private static string g_TestBootstrapDir;
        private static string g_TestOutputDir;
        private static string g_BaselinesDir;
        private static string g_ServiceUrl;
        private static string g_ServiceId;
        private static string g_SvcutilNugetFeed;
        private static string g_SvcutilPkgVersion;
        private static string g_RepositoryRoot;
        private static string g_SdkVersion;

        private static bool containerAlreadyRunning;

        // test group-specific variables.
        private string this_TestGroupBaselinesDir;
        private string this_TestGroupOutputDir;
        private string this_TestGroupBootstrapDir;

        // test case-specific variables
        private MSBuildProj this_TestCaseProject;
        private string this_TestCaseName;
        private string this_TestCaseBaselinesDir;
        private string this_TestCaseOutputDir;
        private string this_TestCaseBootstrapDir;
        private string this_TestCaseSdkVersion;
        private string this_TestCaseLogFile;
        private ILogger this_TestCaseLogger;
        private FixupUtil this_FixupUtil;

        public TestContext TestContext { get; set; }

        private static MSBuildProj __g_StarterProject;
        private static MSBuildProj g_StarterProject
        {
            get
            {
        ...
        private string this_TestGroupProjectName { get { return $"{TestContext.TestName}_TestGroup.csproj"; } }

        private MSBuildProj __this_TestGroupProject;
        private MSBuildProj this_TestGroupProject
        {
            get
            {
                if (TestContext.Properties.ContainsKey($"{TestContext.TestName}.Project"))
                {
                    __this_TestGroupProject = TestContext.Properties[$"{TestContext.TestName}.Project"] as MSBuildProj;
                }

                if (__this_TestGroupProject == null)
                {
                    var fileName = Path.GetFileNameWithoutExtension(this_TestGroupProjectName);
                    var filePath = Path.Combine(this_TestGroupOutputDir, this_TestGroupProjectName);

                    if (!File.Exists(filePath))
                    {
                        // copy project to avoid 'dotnet new + restore' and save a few seconds.
                        CopyProject(g_StarterProject, this_TestGroupOutputDir, fileName);
                    }
                    ...
                    TestContext.Properties[$"{TestContext.TestName}.Project"] = __this_TestGroupProject;
                }

                return __this_TestGroupProject;
            }
        }

        private static string CopyProject(MSBuildProj project, string outputDir, string projectName)
        {
        ...
        public static void ClassInitialize(TestContext testContext)
        {
            ...
            containerAlreadyRunning = DockerHelper.IsContainerRunning();

            if (!containerAlreadyRunning)
            {
                DockerHelper.StartContainer(vsTestsRoot);
            }

            g_SdkVersion = "3.1.100";

            g_ServiceUrl = DockerHelper.GetServiceContainerUrl();
            testContext.WriteLine("Service URL is: '" + g_ServiceUrl + "'.");
            Assert.IsFalse(string.IsNullOrEmpty(g_ServiceUrl), "Could not get Docker container URL!");

            g_ServiceId = DockerHelper.GetServiceContainerId();
            Assert.IsFalse(string.IsNullOrEmpty(g_ServiceId), $"{nameof(g_ServiceId)} is not initialized!");
            testContext.WriteLine("Service ID is: '" + g_ServiceId + "'.");

            g_TestOutputDir = Path.Combine(Directory.GetCurrentDirectory(), "TestOutput");
            ...
            Assert.IsTrue(Directory.Exists(g_RepositoryRoot), $"{nameof(g_RepositoryRoot)} is not initialized!");
            Assert.IsTrue(Directory.Exists(g_TestCasesDir), $"{nameof(g_TestCasesDir)} is not initialized!");
            Assert.IsTrue(Directory.Exists(g_TestResultsDir), $"{nameof(g_TestResultsDir)} is not initialized!");
            Assert.IsTrue(Directory.Exists(g_TestBootstrapDir), $"{nameof(g_TestBootstrapDir)} is not initialized!");
            Assert.IsTrue(Directory.Exists(g_TestOutputDir), $"{nameof(g_TestOutputDir)} is not initialized!");
            Assert.IsTrue(Directory.Exists(g_BaselinesDir), $"{nameof(g_BaselinesDir)} is not initialized!");
            Assert.IsTrue(Directory.Exists(g_TestCasesDir), $"{nameof(g_TestCasesDir)} is not initialized!");
            ...
            Assert.IsTrue(File.Exists(g_StarterProject.FullPath), $"{nameof(g_StarterProject)} is not initialized!");

            Assert.IsFalse(string.IsNullOrEmpty(g_SvcutilNugetFeed), $"{nameof(g_SvcutilNugetFeed)} is not initialized!");
            Assert.IsFalse(string.IsNullOrEmpty(g_SvcutilPkgVersion), $"{nameof(g_SvcutilPkgVersion)} is not initialized!");

            // copy MSBuild global props file to test folder to disable build customizations for test projects.
            ...
            ret = ProcessRunner.TryRunAsync("dotnet", $"tool install --global --add-source {Directory.GetParent(currentDirectory)} dotnet-svcutil --version {g_SvcutilPkgVersion}", currentDirectory, null, CancellationToken.None).Result;
            Assert.IsTrue(ret.ExitCode == 0, "Could not install the global tool." + Environment.NewLine + ret.OutputText);
        }

        [ClassCleanup]
        public static void ClassCleanup()
        {
            // If the container wasn't running when we started the tests, then we started it ourselves.
            // Stop the container to clean up after ourselves.
            if (!containerAlreadyRunning)
            {
                // let the container run continuosly to increase the chances of sucess.
                //DockerHelper.StopContainer(GetRootVStestName());
            }
        }
        ...
        public void TestInitialize()
        {
            ...
            Assert.IsNotNull(testName, $"{nameof(testName)} is not initialized!");

            this_TestGroupBaselinesDir = Path.Combine(g_BaselinesDir, testName);
            this_TestGroupOutputDir = Path.Combine(g_TestResultsDir, testName);
            this_TestGroupBootstrapDir = Path.Combine(g_TestBootstrapDir, testName);

            if (!TestContext.Properties.ContainsKey($"{testName}"))
            {
                ...
                TestContext.Properties[$"{testName}"] = this_TestGroupOutputDir;
            }

            Assert.IsTrue(Directory.Exists(this_TestGroupOutputDir), $"{nameof(this_TestGroupOutputDir)} is not initialized!");
            Assert.IsTrue(Directory.Exists(this_TestGroupBaselinesDir), $"{nameof(this_TestGroupBaselinesDir)} is not initialized!");
        }
        ...
        public void InitializeE2E(string testCaseName, bool createUniqueProject = false, string targetFramework = null, string sdkVersion = null)
        {
        ...
        {
            Assert.IsFalse(string.IsNullOrWhiteSpace(testCaseName), $"{nameof(testCaseName)} is not initialized!");

            this_TestCaseName = testCaseName;
            this_TestCaseOutputDir = Path.Combine(this_TestGroupOutputDir, this_TestCaseName);
            ...
            this_TestCaseBootstrapDir = Path.Combine(this_TestGroupBootstrapDir, this_TestCaseName);

            this_TestCaseLogger = new DebugLogger();
            this_TestCaseLogFile = Path.Combine(this_TestCaseOutputDir, $"{this_TestCaseName}.log");
            ...
            if (createProject == true)
            {
                ...
                var projectPath = CopyProject(g_StarterProject, this_TestCaseOutputDir, this_TestCaseName);
            ...
            this_TestCaseBaselinesDir = Path.Combine(this_TestGroupBaselinesDir, this_TestCaseName);
            ...
            Assert.IsTrue(Directory.Exists(this_TestCaseOutputDir), $"{nameof(this_TestGroupOutputDir)} is not initialized!");
            Assert.IsTrue(Directory.Exists(this_TestCaseBaselinesDir), $"{nameof(this_TestCaseBaselinesDir)} is not initialized!");
            Assert.IsTrue(this_TestCaseBootstrapDir != null, $"{nameof(this_TestCaseBootstrapDir)} is not initialized!");
            Assert.IsTrue(this_TestCaseLogger != null, $"{nameof(this_TestGroupOutputDir)} is not initialized!");
            Assert.IsTrue(this_FixupUtil != null, $"{nameof(this_FixupUtil)} is not initialized!");
        }
        #endregion

        private string AppendCommonOptions(string options)
        {
        ...
        private void ValidateTest(string options, string workingDir, int exitCode, string outputText, bool expectSuccess = true)
        {
            ...
            Assert.IsTrue(isTestSucess, $"{Environment.NewLine}Test failed:{Environment.NewLine}{outputText}{g_GeneralErrMsg}");
            ...
            Assert.IsTrue(validationSuccess, $"{Environment.NewLine}Test failed validation!{failureMessage}{g_GeneralErrMsg}");
        }

        private bool ValidateOutputs(string options, out string failureMessage)
        {
            return ValidateOutputs(options, true, out failureMessage);
        }

        private bool ValidateOutputs(string options, bool isSuccess, out string failureMessage)
        {
            var linuxBaselinePath = Path.Combine(this_TestCaseBaselinesDir, "Linux");
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux) && File.Exists(Path.Combine(linuxBaselinePath, Path.GetFileName(this_TestCaseLogFile))))
            {
        ...
        private bool ValidateFiles(string outputDir, string baselineDir, string bootstrapDir, List<string> generatedFiles, List<string> baselineFiles, out string failureMessage)
        {
        ...
        private string GenerateBaselineDeleteScript(IEnumerable<string> unvalidatedBaselines)
        {
        ...
        private bool CompareFiles(string baselineFile, string generatedFile, out KeyValuePair<string, string>? diffLines)
        {
            diffLines = null;
        ...
        private void WriteLog(string msg, string options = null)
        {
            if (options != null)
            {
                WriteParams(options);
        ...
        private void WriteParams(string options)
        {
        ...
        private static string GetSTestRootDir()
        {
            ...
            if (!Directory.Exists(Path.Combine(vstestDir, "TestCases")))
            {
                ...
                Assert.IsTrue(Directory.Exists(vstestDir), $"{nameof(vstestDir)} is not initialized!");
            }
            return vstestDir;
        }
        ...
        internal static string GetSvcutilPkgVersionAndFeed()
        {
            if (g_SvcutilPkgVersion == null)
            {
                ...
                Assert.IsTrue(nugetFiles.Length <= 1, "There should be one and only one nuget package for testing.");

                if (nugetFiles.Length == 0)
                {
                    var binDir = Path.Combine(g_RepositoryRoot, @"artifacts\Packages\Debug\Shipping");
                    ...
                    Assert.IsNotNull(nugetFile, "There should be at least one nuget package for testing.");
        ...
        internal static string FindRepositoryRoot()
        {
            if (g_RepositoryRoot == null)
            {
                if (string.IsNullOrEmpty(g_RepositoryRoot))
                {
                    ...
                    Assert.IsTrue(rootSolutionFolder != null && File.Exists(rootSolutionFolder), $"Unable to find dotnet-svcutil.sln file, current dir: {parentDir}");
        ...
        private const string globalConfigFormat = "{{ \"sdk\": {{ \"version\": \"{0}\" }} }}";
...
"</configuration>" + Environment.NewLine;
        ...
        {
            private string _cashedWorkingDirectory;
