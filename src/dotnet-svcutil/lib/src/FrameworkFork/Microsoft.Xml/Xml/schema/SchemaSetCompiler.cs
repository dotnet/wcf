...
namespace Microsoft.Xml.Schema {
    ...
    internal sealed class Compiler : BaseProcessor {
            ...
            if (baseType != null && baseType != XmlSchemaComplexType.AnyType && complexType.DerivedBy == XmlSchemaDerivationMethod.Restriction) {
                ...
                if (!IsValidRestriction(derivedParticle, baseParticle)) {
                    ...
                    if (restrictionErrorMsg != null) {
                        SendValidationEvent(ResXml.Sch_InvalidParticleRestrictionDetailed, restrictionErrorMsg, complexType);
                    }
                    else {
        ...
        private void CheckParticleDerivation(XmlSchemaParticle derivedParticle, XmlSchemaParticle baseParticle) {
            ...
            if (!IsValidRestriction(derivedParticle, baseParticle)) {
                if (restrictionErrorMsg != null) {
                    SendValidationEvent(ResXml.Sch_InvalidParticleRestrictionDetailed, restrictionErrorMsg, derivedParticle);
                }
                else {
        ...
        private bool IsValidRestriction(XmlSchemaParticle derivedParticle, XmlSchemaParticle baseParticle) {
            ...
            if (baseParticle is XmlSchemaElement) {
                ...
                if (newBaseParticle is XmlSchemaChoice) { //Base Element is subs grp head.
                    ...
                    restrictionErrorMsg = ResXml.GetString(ResXml.Sch_ForbiddenDerivedParticleForElem);
                    return false;
                }
            }
                ...
                if (derivedParticle is XmlSchemaElement) {
                    ...
                    restrictionErrorMsg = ResXml.GetString(ResXml.Sch_ForbiddenDerivedParticleForChoice); 
                }
                return false;
            }
                ...
                if (derivedParticle is XmlSchemaElement) {
                    ...
                    restrictionErrorMsg = ResXml.GetString(ResXml.Sch_ForbiddenDerivedParticleForSeq); 
                }
                return false;
            }
            else {
        ...
        private bool IsElementFromElement(XmlSchemaElement derivedElement, XmlSchemaElement baseElement) {
            ...
            if(! ((derivedElement.QualifiedName == baseElement.QualifiedName) &&
                ...
                restrictionErrorMsg = ResXml.GetString(ResXml.Sch_ElementFromElement, derivedElement.QualifiedName, baseElement.QualifiedName);    
                return false;
            }
            return true;
        }
        ...
        private bool IsElementFromAny(XmlSchemaElement derivedElement, XmlSchemaAny baseAny) {
            if (!baseAny.Allows(derivedElement.QualifiedName)) {
                restrictionErrorMsg = ResXml.GetString(ResXml.Sch_ElementFromAnyRule1, derivedElement.QualifiedName.ToString());
                return false;
            }
            if (!IsValidOccurrenceRangeRestriction(derivedElement, baseAny)) {
                restrictionErrorMsg = ResXml.GetString(ResXml.Sch_ElementFromAnyRule2, derivedElement.QualifiedName.ToString());
                return false;
            }
            return true;
        }
        ...
        private bool IsAnyFromAny(XmlSchemaAny derivedAny, XmlSchemaAny baseAny) {
            if (!IsValidOccurrenceRangeRestriction(derivedAny, baseAny)) {
                restrictionErrorMsg = ResXml.GetString(ResXml.Sch_AnyFromAnyRule1);
                return false;
            }
            if (!NamespaceList.IsSubset(derivedAny.NamespaceList, baseAny.NamespaceList)) {
                restrictionErrorMsg = ResXml.GetString(ResXml.Sch_AnyFromAnyRule2);
                return false;
            }
            if ((int)derivedAny.ProcessContentsCorrect < (int)baseAny.ProcessContentsCorrect) {
                restrictionErrorMsg = ResXml.GetString(ResXml.Sch_AnyFromAnyRule3);
                return false;
            }
            return true;
        }
        ...
        private bool IsGroupBaseFromAny(XmlSchemaGroupBase derivedGroupBase, XmlSchemaAny baseAny) {
            ...
            for (int i = 0; i < derivedGroupBase.Items.Count; ++i) {
                if (!IsValidRestriction((XmlSchemaParticle)derivedGroupBase.Items[i], baseAny)) {
                    restrictionErrorMsg = ResXml.GetString(ResXml.Sch_GroupBaseFromAny1);
        ...
        private bool IsChoiceFromChoiceSubstGroup(XmlSchemaChoice derivedChoice, XmlSchemaChoice baseChoice) {
            if (!IsValidOccurrenceRangeRestriction(derivedChoice, baseChoice)) {
                restrictionErrorMsg = ResXml.GetString(ResXml.Sch_GroupBaseRestRangeInvalid);
                return false;
            }
        ...
        private bool IsGroupBaseFromGroupBase(XmlSchemaGroupBase derivedGroupBase, XmlSchemaGroupBase baseGroupBase,  bool skipEmptableOnly) {
            ...
            if (derivedGroupBase.Items.Count > baseGroupBase.Items.Count) {
                restrictionErrorMsg = ResXml.GetString(ResXml.Sch_GroupBaseRestNoMap);
                return false;
            }
            int count = 0;
            for (int i = 0; i < baseGroupBase.Items.Count; ++i) {
                ...
                if ((count < derivedGroupBase.Items.Count)
                    ...
                    if (restrictionErrorMsg == null) { //If restriction failed on previous check, do not overwrite error 
                        restrictionErrorMsg = ResXml.GetString(ResXml.Sch_GroupBaseRestNotEmptiable);
                    }
                    return false;
                }
            }
