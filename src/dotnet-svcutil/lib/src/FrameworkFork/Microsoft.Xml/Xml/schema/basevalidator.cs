    ...
    internal class BaseValidator
    {
        ...
        public void ValidateText()
        {
            if (context.NeedValidateChildren)
            {
                ...
                if (contentType == XmlSchemaContentType.ElementOnly)
                {
                    ...
                    if (names == null)
                    {
                        SendValidationEvent(ResXml.Sch_InvalidTextInElement, XmlSchemaValidator.BuildElementName(context.LocalName, context.Namespace));
                    }
                    else
                    {
                        Debug.Assert(names.Count > 0);
                        SendValidationEvent(ResXml.Sch_InvalidTextInElementExpecting, new string[] { XmlSchemaValidator.BuildElementName(context.LocalName, context.Namespace), XmlSchemaValidator.PrintExpectedElements(names, false) });
                    }
                }
        ...
        protected static void ProcessEntity(SchemaInfo sinfo, string name, object sender, ValidationEventHandler eventhandler, string baseUri, int lineNumber, int linePosition)
        {
            ...
            if (!sinfo.GeneralEntities.TryGetValue(new XmlQualifiedName(name), out en))
            {
                ...
                e = new XmlSchemaException(ResXml.Sch_UndeclaredEntity, name, baseUri, lineNumber, linePosition);
            }
            else if (en.NData.IsEmpty)
            {
                e = new XmlSchemaException(ResXml.Sch_UnparsedEntityRef, name, baseUri, lineNumber, linePosition);
            }
            if (e != null)
            {
        ...
        protected static void ProcessEntity(SchemaInfo sinfo, string name, IValidationEventHandling eventHandling, string baseUriStr, int lineNumber, int linePosition)
        {
            ...
            {
                // validation error, see xml spec [68]
                errorResId = ResXml.Sch_UndeclaredEntity;
            }
            else if (en.NData.IsEmpty)
            {
                errorResId = ResXml.Sch_UnparsedEntityRef;
            }
            if (errorResId != null)
            {
