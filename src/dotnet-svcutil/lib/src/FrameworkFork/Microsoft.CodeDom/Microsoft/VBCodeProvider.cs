...
namespace Microsoft.VisualBasic
{
    ...
    internal class VBCodeGenerator : CodeCompiler
    {
        ...
        private static volatile Regex s_outputReg;
        ...
        {
            FixedStringLookup.VerifyLookupTable(s_keywords, true);
        ...
        protected override void ProcessCompilerOutputLine(CompilerResults results, string line)
        {
            if (s_outputReg == null)
            {
                s_outputReg = new Regex(@"^([^(]*)\(?([0-9]*)\)? ?:? ?(error|warning) ([A-Z]+[0-9]+) ?: ((.|\n)*)");
            }
            Match m = s_outputReg.Match(line);
            if (m.Success)
            {
        ...
        protected override string CmdArgsFromParameters(CompilerParameters options)
        {
            ...
            foreach (string s in options.ReferencedAssemblies)
            {
                ...
                {
                    throw new ArgumentException("SRCodeDom.NullOrEmpty_Value_in_Property" + "ReferencedAssemblies", "options");
                }
            }
        ...
        protected override CompilerResults FromFileBatch(CompilerParameters options, string[] fileNames)
        {
            ...
            if (options.OutputAssembly == null || options.OutputAssembly.Length == 0)
            {
                ...
                // we can later access with our current credential.<BUGBUG>If we don't do this, the compiler
                // could end up creating an assembly that we cannot open (bug ASURT 83492)</BUGBUG>
    ...
    internal class VBMemberAttributeConverter : VBModifierAttributeConverter
    {
        ...
        private static volatile VBMemberAttributeConverter s_defaultConverter;
        ...
        public static VBMemberAttributeConverter Default
        {
            get
            {
                ...
                {
                    s_defaultConverter = new VBMemberAttributeConverter();
                }
    ...
    {
        private static volatile VBTypeAttributeConverter s_defaultConverter;
        private static volatile string[] s_names;
        private static volatile object[] s_values;
        ...
        public static VBTypeAttributeConverter Default
        {
            get
            {
                if (s_defaultConverter == null)
                {
                    s_defaultConverter = new VBTypeAttributeConverter();
                }
                return s_defaultConverter;
            }
        }
    ...
    internal abstract class VBModifierAttributeConverter : TypeConverter
    {
        ...
        public override object ConvertTo(ITypeDescriptorContext context, CultureInfo culture, object value, Type destinationType)
        {
            ...
            if (destinationType == typeof(string))
            {
                ...
                return string.Format(SRCodeDom.toStringUnknown);
            }
