...
namespace System.ServiceModel.Channels
{
    public abstract class MessageFault
    {
            ...
            using (reader)
            {
                ...
                catch (InvalidOperationException e)
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new CommunicationException(
                        SRServiceModel.SFxErrorDeserializingFault, e));
                }
        ...
        public T GetDetail<T>(XmlObjectSerializer serializer)
        {
            ...
            if (!reader.EOF)
            {
                ...
                if (reader.NodeType != XmlNodeType.EndElement && !reader.EOF)
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new FormatException(SRServiceModel.ExtraContentIsPresentInFaultDetail));
            }
            return value;
        }
        ...
        {
            if (!HasDetail)
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(SRServiceModel.FaultDoesNotHaveAnyDetail));
    ...
    internal class ReceivedFault : MessageFault
    {
        ...
        private static ReceivedFault CreateFault12Driver(XmlDictionaryReader reader, int maxBufferSize, EnvelopeVersion version)
        {
            ...
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new FormatException(SRServiceModel.AtLeastOneFaultReasonMustBeSpecified));
            }
            else
            {
