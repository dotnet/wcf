...
namespace System
{
    internal class UriTemplateTrieNode
    {
            ...
            else
            {
                ...
                if (curWireSeg.EndsWithSlash)
                {
                        ...
                        currentLocation.node._nextLiteralSegment.ContainsKey(curWireSeg))
                    {
                        ...
                        AscendingSortedCompoundSegmentsCollection<UriTemplateTrieLocation>.Lookup(currentLocation.node._nextCompoundSegment, curWireSeg, out compoundLocationsSet))
                    {
                ...
                else
                {
                        ...
                        currentLocation.node._finalLiteralSegment.ContainsKey(curWireSeg))
                    {
                        ...
                        AscendingSortedCompoundSegmentsCollection<UriTemplatePathPartiallyEquivalentSet>.Lookup(currentLocation.node._finalCompoundSegment, curWireSeg, out compoundPathEquivalentSets))
                    {
            ...
            while (node != null)
            {
                ...
                if (node._finalLiteralSegment != null)
                {
                    UriTemplatePathPartiallyEquivalentSet pes =
                        GetAnyDictionaryValue<UriTemplatePathPartiallyEquivalentSet>(node._finalLiteralSegment);
                ...
                if (node._nextLiteralSegment != null)
                {
                    UriTemplateTrieLocation location =
                        GetAnyDictionaryValue<UriTemplateTrieLocation>(node._nextLiteralSegment);
                    node = location.node;
                }
        ...
        private class AscendingSortedCompoundSegmentsCollection<T>
            where T : class
        {
