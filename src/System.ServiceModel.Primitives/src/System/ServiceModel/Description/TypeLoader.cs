...
namespace System.ServiceModel.Description
{
    internal class TypeLoader
    {
                ...
                for (Type service = actualContractType.BaseType(); service != null; service = service.BaseType())
                {
                    if (ServiceReflector.GetSingleAttribute<ServiceContractAttribute>(service) != null)
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(
                            SRP.Format(SRP.SFxContractInheritanceRequiresInterfaces, actualContractType, service)));
                    }
                }
            }
        }
                    ...
                    if (operationContractProviderType != null)
                    {
                        if (operationContractProviderType == s_OperationContractAttributeType)
                        {
                            throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(SRP.Format(
                                SRP.SFxOperationContractOnNonServiceContract, methodInfo.Name, aParentType.Name)));
                        }
                        else
                        {
                            throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(SRP.Format(
                                SRP.SFxOperationContractProviderOnNonServiceContract, operationContractProviderType.Name, methodInfo.Name, aParentType.Name)));
                        }
                    }
                }
            }
        ...
        private IEnumerable<Type> GetKnownTypes(object[] knownTypeAttributes, ICustomAttributeProvider provider)
        {
            if (knownTypeAttributes.Length == 1)
            {
                ...
                if (!string.IsNullOrEmpty(knownTypeAttribute.MethodName))
                {
                    ...
                    if (method == null)
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(SRP.Format(SRP.SFxKnownTypeAttributeUnknownMethod3, provider, knownTypeAttribute.MethodName, type.FullName)));
                    }

                    if (!typeof(IEnumerable<Type>).IsAssignableFrom(method.ReturnType))
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(SRP.Format(SRP.SFxKnownTypeAttributeReturnType3, provider, knownTypeAttribute.MethodName, type.FullName)));
                    }
            ...
            for (int i = 0; i < knownTypeAttributes.Length; ++i)
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(SRP.Format(SRP.SFxKnownTypeAttributeInvalid1, provider.ToString())));
                }
                ...
                else
                {
                    if (opDesc.SyncMethod != null && opDesc.BeginMethod != null)
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                            new InvalidOperationException(SRP.Format(SRP.SyncAsyncMatchConsistency_Attributes6,
                    ...
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                            new InvalidOperationException(SRP.Format(SRP.SyncTaskMatchConsistency_Attributes6,
                                                                       opDesc.SyncMethod.Name,
                                                                       opDesc.SyncMethod.DeclaringType,
                    ...
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                            new InvalidOperationException(SRP.Format(SRP.TaskAsyncMatchConsistency_Attributes6,
                                                                       opDesc.TaskMethod.Name,
        ...
        internal static void EnsureCallbackType(Type callbackType)
        {
            ...
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentException(SRP.Format(SRP.SFxInvalidCallbackContractType, callbackType.Name)));
            }
        }
        ...
        internal static void EnsureSubcontract(ServiceContractAttribute svcContractAttr, Type contractType)
        {
            ...
            for (int i = 0; i < types.Count; i++)
            {
                ...
                if (inheritedContractAttr.CallbackContract != null)
                {
                    ...
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(
                            SRP.Format(SRP.InAContractInheritanceHierarchyIfParentHasCallbackChildMustToo,
                    ...
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(
                            SRP.Format(SRP.InAContractInheritanceHierarchyTheServiceContract3_2,
                ...
                foreach (OperationDescription op in inheritedContractDescription.Operations)
                {
                    if (!contractDescription.Operations.Contains(op)) // in a diamond hierarchy, ensure we don't add same op twice from two different parents
                    {
                        ...
                        foreach (OperationDescription existingOp in existingOps)
                        {
                            ...
                            {
                                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(
                                    SRP.Format(SRP.CannotInheritTwoOperationsWithTheSameName3,
        ...
        private void CheckDuplicateFaultContract(FaultDescriptionCollection faultDescriptionCollection, FaultDescription fault, string operationName)
        {
            foreach (FaultDescription existingFault in faultDescriptionCollection)
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(
                        SRP.Format(SRP.SFxFaultContractDuplicateDetailType, operationName, fault.DetailType)));
                }
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(
                        SRP.Format(SRP.SFxFaultContractDuplicateElement, operationName, fault.ElementName, fault.Namespace)));
                }
            }
        }
                                                           ...
                                                           MessageDirection direction)
        {
            ...
            for (int i = 0; i < messageDescription.Headers.Count; i++)
            {
                ...
                if (header.IsUnknownHeaderCollection)
                {
                    ...
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(SRP.Format(SRP.SFxMultipleUnknownHeaders, methodInfo, methodInfo.DeclaringType)));
                    }
                    else
                    {
                                                  ...
                                                  MessageDirection direction)
        {
            foreach (ParameterInfo param in parameters)
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(SRP.Format(SRP.SFxInvalidMessageContractSignature, methodName)));
                }
            }
            ...
            for (int index = 0; index < parameters.Length; index++)
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidMessageContractException(SRP.Format(SRP.SFxDuplicateMessageParts, partDescription.Name, partDescription.Namespace)));
                }
                                                  ...
                                                  MessageDirection direction)
        {
            ...
            for (Type baseType = typedMessageType; baseType != null && baseType != typeof(object) && baseType != typeof(ValueType); baseType = baseType.BaseType())
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(SRP.Format(SRP.SFxMessageContractBaseTypeNotValid, baseType, typedMessageType)));
                }
                                                                    ...
                                                                    int serializationPosition)
        {
            ...
            if (headerAttr is MessageHeaderArrayAttribute)
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(SRP.Format(SRP.SFxInvalidMessageHeaderArrayType, defaultName)));
                }
            ...
            if (headerDescription.TypedHeader)
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(SRP.Format(SRP.SFxStaticMessageHeaderPropertiesNotAllowed, defaultName)));
                }
            }
            else
            {
        ...
        private class SyncAsyncOperationConsistencyVerifier : OperationConsistencyVerifier
        {
            ...
            public override void VerifyParameterLength()
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                        new InvalidOperationException(SRP.Format(SRP.SyncAsyncMatchConsistency_Parameters5,
            ...
            public override void VerifyReturnType()
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                        new InvalidOperationException(SRP.Format(SRP.SyncAsyncMatchConsistency_ReturnType5,
                                                                   _syncOperation.SyncMethod.Name,
                                                                   _syncOperation.SyncMethod.DeclaringType,
                                                                   _asyncOperation.BeginMethod.Name,
                                                                   _asyncOperation.EndMethod.Name,
            ...
            public override void VerifyIsOneWayStatus()
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                            new InvalidOperationException(SRP.Format(SRP.SyncAsyncMatchConsistency_Property6,
                                                                       _syncOperation.SyncMethod.Name,
                                                                       _syncOperation.SyncMethod.DeclaringType,
                                                                       _asyncOperation.BeginMethod.Name,
                                                                       _asyncOperation.EndMethod.Name,
        ...
        private class SyncTaskOperationConsistencyVerifier : OperationConsistencyVerifier
        {
            ...
            public override void VerifyParameterLength()
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                        new InvalidOperationException(SRP.Format(SRP.SyncTaskMatchConsistency_Parameters5,
                                                                   _syncOperation.SyncMethod.Name,
                                                                   _syncOperation.SyncMethod.DeclaringType,
            ...
            public override void VerifyReturnType()
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                        new InvalidOperationException(SRP.Format(SRP.SyncTaskMatchConsistency_ReturnType5,
                                                                   _syncOperation.SyncMethod.Name,
                                                                   _syncOperation.SyncMethod.DeclaringType,
                                                                   _taskOperation.TaskMethod.Name,
                                                                   _syncOperation.Name)));
            ...
            public override void VerifyIsOneWayStatus()
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                            new InvalidOperationException(SRP.Format(SRP.SyncTaskMatchConsistency_Property6,
                                                                       _syncOperation.SyncMethod.Name,
                                                                       _syncOperation.SyncMethod.DeclaringType,
                                                                       _taskOperation.TaskMethod.Name,
                                                                       _syncOperation.Name,
        ...
        private class TaskAsyncOperationConsistencyVerifier : OperationConsistencyVerifier
        {
            ...
            public override void VerifyParameterLength()
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                        new InvalidOperationException(SRP.Format(SRP.TaskAsyncMatchConsistency_Parameters5,
                                                                   _taskOperation.TaskMethod.Name,
            ...
            public override void VerifyReturnType()
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                        new InvalidOperationException(SRP.Format(SRP.TaskAsyncMatchConsistency_ReturnType5,
                                                                   _taskOperation.TaskMethod.Name,
                                                                   _taskOperation.TaskMethod.DeclaringType,
                                                                   _asyncOperation.BeginMethod.Name,
                                                                   _asyncOperation.EndMethod.Name,
            ...
            public override void VerifyIsOneWayStatus()
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                            new InvalidOperationException(SRP.Format(SRP.TaskAsyncMatchConsistency_Property6,
                                                                       _taskOperation.TaskMethod.Name,
                                                                       _taskOperation.TaskMethod.DeclaringType,
                                                                       _asyncOperation.BeginMethod.Name,
                                                                       _asyncOperation.EndMethod.Name,
