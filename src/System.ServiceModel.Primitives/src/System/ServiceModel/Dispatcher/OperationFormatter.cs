...
namespace System.ServiceModel.Dispatcher
{
    internal abstract class OperationFormatter : IClientMessageFormatter, IDispatchMessageFormatter
    {
        ...
        public object DeserializeReply(Message message, object[] parameters)
        {
            ...
            catch (XmlException xe)
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new CommunicationException(
                    SRP.Format(SRP.SFxErrorDeserializingReplyBodyMore, _operationName, xe.Message), xe));
            }
            catch (FormatException fe)
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new CommunicationException(
                    SRP.Format(SRP.SFxErrorDeserializingReplyBodyMore, _operationName, fe.Message), fe));
            }
            catch (SerializationException se)
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new CommunicationException(
                    SRP.Format(SRP.SFxErrorDeserializingReplyBodyMore, _operationName, se.Message), se));
            }
        }
        ...
        private static object CreateTypedMessageInstance(Type messageContractType)
        {
            ...
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(SRP.Format(SRP.SFxMessageContractRequiresDefaultConstructor, messageContractType.FullName), mme));
            }
        }
            ...
            catch (XmlException xe)
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                    OperationFormatter.CreateDeserializationFailedFault(
                        SRP.Format(SRP.SFxErrorDeserializingRequestBodyMore, _operationName, xe.Message),
                        xe));
            }
            catch (FormatException fe)
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                    OperationFormatter.CreateDeserializationFailedFault(
                        SRP.Format(SRP.SFxErrorDeserializingRequestBodyMore, _operationName, fe.Message),
                        fe));
            }
            catch (SerializationException se)
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new CommunicationException(
                    SRP.Format(SRP.SFxErrorDeserializingRequestBodyMore, _operationName, se.Message),
                    se));
            }
        }
        ...
        internal static void Validate(OperationDescription operation, bool isRpc, bool isEncoded)
        {
            if (isEncoded && !isRpc)
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(SRP.Format(SRP.SFxDocEncodedNotSupported, operation.Name)));
            }
            ...
            for (int i = 0; i < operation.Messages.Count; i++)
            {
                ...
                if (message.IsTypedMessage || message.IsUntypedMessage)
                {
                    if (isRpc && operation.IsValidateRpcWrapperName)
                    {
                        if (!isEncoded)
                        {
                            throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(SRP.Format(SRP.SFxTypedMessageCannotBeRpcLiteral, operation.Name)));
                        }
                    }
            ...
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(SRP.Format(SRP.SFxTypedOrUntypedMessageCannotBeMixedWithParameters, operation.Name)));
            }

            if (isRpc && hasTypedOrUntypedMessage && hasVoid)
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(SRP.Format(SRP.SFxTypedOrUntypedMessageCannotBeMixedWithVoidInRpc, operation.Name)));
            }
        }
        ...
        {
            reason = SRP.Format(SRP.SFxDeserializationFailed1, reason);
        ...
        internal class TypedMessageParts
        {
            ...
            public TypedMessageParts(object instance, MessageDescription description)
            {
                ...
                _instance = instance ?? throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentNullException(SRP.Format(SRP.SFxTypedMessageCannotBeNull, description.Action)));
            }
