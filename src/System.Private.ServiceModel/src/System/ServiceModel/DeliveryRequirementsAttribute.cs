    ...
    public sealed class DeliveryRequirementsAttribute : Attribute, IContractBehavior
    {
                ...
                || QueuedDeliveryRequirements == QueuedDeliveryRequirementsMode.NotAllowed)
            {
                ...
                if (caps == null)
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(
                        SR.Format(SR.SinceTheBindingForDoesnTSupportIBindingCapabilities2_1, name)));
                }
                else
                {
                    ...
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(
                            SR.Format(SR.BindingRequirementsAttributeRequiresQueuedDelivery1, name)));
                    }
                    else if (QueuedDeliveryRequirements == QueuedDeliveryRequirementsMode.NotAllowed && queuedTransport)
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(
                            SR.Format(SR.BindingRequirementsAttributeDisallowsQueuedDelivery1, name)));
                    }
                }
            }
        }
        ...
        private void EnsureOrderedDeliveryRequirements(string name, Binding binding)
        {
            if (RequireOrderedDelivery)
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(
                        SR.Format(SR.SinceTheBindingForDoesnTSupportIBindingCapabilities1_1, name)));
                }
                else
                {
                    ...
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(
                            SR.Format(SR.TheBindingForDoesnTSupportOrderedDelivery1, name)));
                    }
                }
            }
        }
