...
namespace System.ServiceModel.Security
{
    internal class IssuanceTokenProviderState : IDisposable
    {
        ...
        public void SetServiceToken(GenericXmlSecurityToken serviceToken)
        {
            ...
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(SR.NegotiationIsCompleted));
            }
        ...
        private void CheckCompleted()
        {
            if (!IsNegotiationCompleted)
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(SR.NegotiationIsNotCompleted));
            }
        }
    }
}
