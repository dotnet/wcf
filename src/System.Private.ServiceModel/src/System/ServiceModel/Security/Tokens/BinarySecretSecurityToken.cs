...
namespace System.ServiceModel.Security.Tokens
{
    public class BinarySecretSecurityToken : SecurityToken
    {
        ...
        protected BinarySecretSecurityToken(string id, int keySizeInBits, bool allowCrypto)
        {
            ...
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentOutOfRangeException(nameof(keySizeInBits), SR.Format(SR.ValueMustBeInRange, 0, 512)));
            }

            if ((keySizeInBits % 8) != 0)
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentOutOfRangeException(nameof(keySizeInBits), SR.Format(SR.KeyLengthMustBeMultipleOfEight, keySizeInBits)));
            }
