...
namespace System.ServiceModel.Security
{
    ...
    internal sealed class SecuritySessionClientSettings<TChannel> : IChannelSecureConversationSessionSettings, ISecurityCommunicationObject
    {
        ...
        public Task OnOpenAsync(TimeSpan timeout)
        {
            ...
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(SR.Format(SR.SecurityStandardsManagerNotSet, GetType().ToString())));
            }
        ...
        private abstract class ClientSecuritySessionChannel : ChannelBase
        {
            ...
            private void ProcessCloseResponse(Message response)
            {
                ...
                {
                    throw TraceUtility.ThrowHelperError(new MessageSecurityException(SR.SessionTokenWasNotClosed), response);
                }
            }
            ...
            private void ProcessCloseMessage(Message message)
            {
                ...
                {
                    throw TraceUtility.ThrowHelperWarning(new MessageSecurityException(SR.NoCloseTargetSpecified), message);
                }
                ...
                else
                {
                    ...
                    lock (ThisLock)
                    {
                        if (IsKeyRenewalNeeded())
                        {
                            ...
                            throw DiagnosticUtility.ExceptionUtility.ThrowHelperWarning(new SessionKeyExpiredException(SR.UnableToRenewSessionKey));
                        }
                    }
                }
            }
            ...
            protected async Task<(bool, bool)> CloseSessionAsync(TimeSpan timeout)
            {
                    ...
                    ServiceModelActivity.CreateBoundedActivity() : null)
                {
                    ...
                    {
                        ServiceModelActivity.Start(activity, SR.ActivitySecurityClose, ActivityType.SecuritySetup);
                    }
            ...
            protected void CheckOutputOpen()
            {
                ...
                lock (ThisLock)
                {
                    if (_isOutputClosed)
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperWarning(new CommunicationException(SR.OutputNotExpected));
                    }
                }
            }
            ...
            protected class SoapSecurityOutputSession : ISecureConversationSession, IOutputSession
            {
                ...
                public string Id
                {
                    get
                    {
                        if (_sessionId == null)
                        {
                            throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(SR.ChannelMustBeOpenedToGetSessionId));
                        }
        ...
        private sealed class SecurityRequestSessionChannel : ClientSecuritySimplexSessionChannel, IAsyncRequestSessionChannel
        {
            ...
            private Message ProcessReply(Message reply, TimeSpan timeout, SecurityProtocolCorrelationState correlationState)
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new MessageSecurityException(SR.SecuritySessionFaultReplyWasSent, new FaultException(protocolFault)));
                }
        ...
        private class ClientSecurityDuplexSessionChannel : ClientSecuritySessionChannel, IAsyncDuplexSessionChannel
        {
            ...
            private class SoapSecurityClientDuplexSession : SoapSecurityOutputSession, IDuplexSession, IAsyncDuplexSession
            {
                ...
                private void CheckInitialized()
                {
                    if (!_initialized)
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(SR.ChannelNotOpen));
                    }
                }
