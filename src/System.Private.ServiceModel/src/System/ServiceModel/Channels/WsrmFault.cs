...
namespace System.ServiceModel.Channels
{
    internal abstract class WsrmFault : MessageFault
    {
        ...
        public virtual CommunicationException CreateException()
        {
            ...
            if (IsRemote)
            {
                ...
                message = SR.Format(SR.WsrmFaultReceived, message);
            }
            else
            {
    ...
    internal sealed class InvalidAcknowledgementFault : WsrmHeaderFault
    {
        ...
        public InvalidAcknowledgementFault(UniqueId sequenceID, SequenceRangeCollection ranges)
            : base(true, WsrmFeb2005Strings.InvalidAcknowledgement, SR.InvalidAcknowledgementFaultReason,
            SR.InvalidAcknowledgementReceived, sequenceID, true, false)
        {
            _ranges = ranges;
        }
    ...
    internal sealed class LastMessageNumberExceededFault : WsrmHeaderFault
    {
        public LastMessageNumberExceededFault(UniqueId sequenceID)
            : base(true, WsrmFeb2005Strings.LastMessageNumberExceeded, SR.LastMessageNumberExceededFaultReason,
            SR.LastMessageNumberExceeded, sequenceID, false, true)
        {
        }
    ...
    internal sealed class MessageNumberRolloverFault : WsrmHeaderFault
    {
        public MessageNumberRolloverFault(UniqueId sequenceID)
            : base(true, WsrmFeb2005Strings.MessageNumberRollover, SR.MessageNumberRolloverFaultReason,
            SR.MessageNumberRollover, sequenceID, true, true)
        {
        }
                ...
                if (reliableMessagingVersion == ReliableMessagingVersion.WSReliableMessaging11)
                {
                        ...
                        || (maxMessageNumber <= 0))
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new XmlException(
                            SR.Format(SR.InvalidSequenceNumber, maxMessageNumber)));
                    }
    ...
    internal sealed class SequenceClosedFault : WsrmHeaderFault
    {
        public SequenceClosedFault(UniqueId sequenceID)
            : base(true, Wsrm11Strings.SequenceClosed, SR.SequenceClosedFaultString,
    ...
    internal sealed class SequenceTerminatedFault : WsrmHeaderFault
    {
        ...
        {
            return CreateCommunicationFault(sequenceId, SR.SequenceTerminatedMaximumRetryCountExceeded, null);
        }
        ...
        {
            return CreateProtocolFault(sequenceID, SR.SequenceTerminatedQuotaExceededException, null);
        }
    }
    ...
    internal sealed class UnknownSequenceFault : WsrmHeaderFault
    {
        public UnknownSequenceFault(UniqueId sequenceID)
            : base(true, WsrmFeb2005Strings.UnknownSequence, SR.UnknownSequenceFaultReason,
            SR.UnknownSequenceMessageReceived, sequenceID, true, true)
        {
        }
        ...
        public override CommunicationException CreateException()
        {
            ...
            {
                message = FaultException.GetSafeReasonText(Reason);
                message = SR.Format(SR.UnknownSequenceFaultReceived, message);
            }
            else
            {
