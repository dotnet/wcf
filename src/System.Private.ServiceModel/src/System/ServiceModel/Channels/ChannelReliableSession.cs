...
namespace System.ServiceModel.Channels
{
    internal abstract class ChannelReliableSession : ISession
    {
        ...
        private void OnInactivityElapsed(object state)
        {
            WsrmFault fault;
            Exception e;
            string exceptionMessage = SR.Format(SR.SequenceTerminatedInactivityTimeoutExceeded, Settings.InactivityTimeout);
            ...
            if (SequenceID != null)
            {
                string faultReason = SR.Format(SR.SequenceTerminatedInactivityTimeoutExceeded, Settings.InactivityTimeout);
    ...
    internal class ClientReliableSession : ChannelReliableSession, IOutputSession
    {
            ...
            base(channel, factory, binder, faultHelper)
        {
            ...
            _requestor.TimeoutString1Index = SR.TimeoutOnOpen;
        ...
        private void ProcessCreateSequenceResponse(Message response, DateTime start)
        {
            ...
            using (response)
            {
                ...
                else
                {
                    ...
                    if (info.ParsingException != null)
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ProtocolException(SR.UnparsableCSResponse, info.ParsingException));
                    ...
                    {
                        exceptionReason = SR.Format(SR.InvalidWsrmResponseChannelNotOpened,
                    ...
                    {
                        exceptionReason = SR.Format(SR.WsrmMessageWithWrongRelatesToExceptionString, WsrmFeb2005Strings.CreateSequence);
                        faultReason = SR.Format(SR.WsrmMessageWithWrongRelatesToFaultString, WsrmFeb2005Strings.CreateSequence);
                    }
                    else if ((createResponse.AcceptAcksTo == null) && (InputID != null))
                    {
                        if (Settings.ReliableMessagingVersion == ReliableMessagingVersion.WSReliableMessagingFebruary2005)
                        {
                            exceptionReason = SR.CSResponseWithoutOffer;
                            faultReason = SR.CSResponseWithoutOfferReason;
                        }
                        else if (Settings.ReliableMessagingVersion == ReliableMessagingVersion.WSReliableMessaging11)
                        {
                            exceptionReason = SR.CSResponseOfferRejected;
                            faultReason = SR.CSResponseOfferRejectedReason;
                        }
                        else
                        {
                    ...
                    {
                        exceptionReason = SR.CSResponseWithOffer;
                        faultReason = SR.CSResponseWithOfferReason;
                    }
                    else if (createResponse.AcceptAcksTo != null && (createResponse.AcceptAcksTo.Uri != _binder.RemoteAddress.Uri))
                    {
                        exceptionReason = SR.AcksToMustBeSameAsRemoteAddress;
                        faultReason = SR.AcksToMustBeSameAsRemoteAddressReason;
                    }
        ...
        protected override WsrmFault VerifyDuplexProtocolElements(WsrmMessageInfo info)
        {
            ...
            if (fault != null)
                ...
                return SequenceTerminatedFault.CreateProtocolFault(OutputID, SR.SequenceTerminatedUnexpectedCS, SR.UnexpectedCS);
            else if (info.CreateSequenceResponseInfo != null && info.CreateSequenceResponseInfo.Identifier != OutputID)
                return SequenceTerminatedFault.CreateProtocolFault(OutputID, SR.SequenceTerminatedUnexpectedCSROfferId, SR.UnexpectedCSROfferId);
            else
                return null;
        }
        ...
        protected override WsrmFault VerifySimplexProtocolElements(WsrmMessageInfo info)
        {
            if (info.AcknowledgementInfo != null && info.AcknowledgementInfo.SequenceID != OutputID)
                ...
                return SequenceTerminatedFault.CreateProtocolFault(OutputID, SR.SequenceTerminatedUnexpectedAckRequested, SR.UnexpectedAckRequested);
            else if (info.CreateSequenceInfo != null)
                return SequenceTerminatedFault.CreateProtocolFault(OutputID, SR.SequenceTerminatedUnexpectedCS, SR.UnexpectedCS);
