...
namespace WcfService
{
    ...
    public interface ITestHost
    {
        ...
        Stream GetClientCert(bool exportAsPem);

        [OperationContract]
        [WebGet(UriTemplate = "GetCrl?serialNum={serialNum}", BodyStyle = WebMessageBodyStyle.Bare)]
        Stream GetCrl(string serialNum);

        [OperationContract]
        [WebGet(UriTemplate = "GetPeerCert?exportAsPem={exportAsPem}", BodyStyle = WebMessageBodyStyle.Bare)]
        byte[] GetPeerCert(bool exportAsPem);

        [OperationContract]
        [WebGet(UriTemplate = "GetRootCert?exportAsPem={exportAsPem}", BodyStyle = WebMessageBodyStyle.Bare)]
        Stream GetRootCert(bool exportAsPem);

        [OperationContract]
        [WebGet(UriTemplate = "GetFqdn", BodyStyle = WebMessageBodyStyle.Bare)]
        Stream GetFQDN();
    ...
    public class TestHost : ITestHost
    {
        ...
        public Stream GetRootCert(bool exportAsPem)
        {
        ...
        public Stream GetFQDN()
        {
            return new MemoryStream(Encoding.UTF8.GetBytes(Dns.GetHostEntry("127.0.0.1").HostName));
        }

        public byte[] GetPeerCert(bool exportAsPem)
        {
            X509Certificate2 peerCert = CertificateFromFriendlyName(StoreName.My, StoreLocation.LocalMachine, "WCF Bridge - Machine certificate generated by the CertificateManager");
            if (exportAsPem)
            {
                return Encoding.ASCII.GetBytes(GetCertificateAsPem(peerCert));
            }
            else
            {
                return peerCert.Export(X509ContentType.Pfx, "test");
            }
        }
